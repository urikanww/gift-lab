...
class ChaseUnansweredOrders extends Command
{
    ...
    public function handle(OrderNotifier $notifier, AuditLogger $audit, ReminderSchedule $schedule): int
    {
        $sent = 0;
        $flagged = 0;
        ...
        foreach ($this->waitingOnPrice() as $quote) {
            ...
            [$didSend, $didFlag] = $this->chase($quote, $since, $schedule->ladder('quote_days', ReminderSchedule::DEFAULT_QUOTE_LADDER), $notifier, $audit, 'price');
            $sent += (int) $didSend;
            $flagged += (int) $didFlag;
        }
        ...
        foreach ($this->waitingOnProof() as $quote) {
            ...
            [$didSend, $didFlag] = $this->chase($quote, $since, $schedule->ladder('proof_days', ReminderSchedule::DEFAULT_PROOF_LADDER), $notifier, $audit, 'proof');
            $sent += (int) $didSend;
            $flagged += (int) $didFlag;
        }
