...
class AppServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        // Courier (NinjaVan): the live HTTP branch is added in Task 5. Until
        // credentials exist, the fixture serves local/testing; production
        // without creds must fail closed (refuse to resolve) rather than
        // silently binding a fake that never dispatches a real parcel.
        $this->app->singleton(CourierClient::class, function ($app) {
            if ($app->environment(['local', 'testing'])) {
                return new FixtureNinjaVanClient();
            }

            throw new RuntimeException('NinjaVan courier client is not configured for the "'.$app->environment().'" environment. The fixture never dispatches a real parcel; configure the live client before shipping in this environment.');
        });
    }
