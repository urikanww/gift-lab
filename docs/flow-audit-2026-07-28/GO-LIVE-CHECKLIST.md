...
| 2.6 | `NINJAVAN_WEBHOOK_SECRET` set **and matches NinjaVan's real signing** | ⚠️ Inbound webhook **fails closed (401)** without a secret. **Verify HOW NinjaVan actually signs its webhooks** (the shared secret + which header) and set `NINJAVAN_WEBHOOK_SECRET` / `NINJAVAN_WEBHOOK_SIGNATURE_HEADER` to match — our endpoint expects `HMAC-SHA256(body)` in `X-Ninja-Hmac` by default. If NinjaVan's scheme differs (or it doesn't sign), real delivery/return events would be rejected and orders would never auto-close. Confirm against a real sandbox webhook before launch. |
